...
namespace stn{
    ...
    public:
        ...
        //网络层收到push消息回调 
        virtual void OnPush(uint64_t channelId, uint32_t cmdId, uint32_t taskId, const AutoBuffer& body, const AutoBuffer& bufExt) = 0;
        //底层获取task要发送的数据 
        virtual bool Req2Buf(uint32_t taskId, void* const userContext, AutoBuffer& outbuffer, AutoBuffer& extend, int& error_code, const int channelSelect) = 0;
        //底层回包返回给上层解析 
        virtual int Buf2Resp(uint32_t taskId, void* const userContext, const AutoBuffer& _inbuffer, const AutoBuffer& bufExt, int& errorCode, const int channelSelect) = 0;
        //任务执行结束 
        virtual int  OnTaskEnd(uint32_t taskId, void* const userContext, int errorType, int errorCode) = 0;


        //上报网络连接状态 
        virtual void ReportConnectStatus(int status, int _longlink_status) = 0;
        virtual void OnLongLinkNetworkError(ErrCmdType errType, int errCode, const std::string& ip, uint16_t port) {}
        virtual void OnShortLinkNetworkError(ErrCmdType errType, int errCode, const std::string& ip, const std::string& host, uint16_t port) {}
        
        virtual void OnLongLinkStatusChange(int status) {}
        //长连信令校验 ECHECK_NOW = 0, ECHECK_NEXT = 1, ECHECK_NEVER = 2 
        virtual int  GetLonglinkIdentifyCheckBuffer(AutoBuffer& _identify_buffer, AutoBuffer& _buffer_hash, int32_t& cmdId) = 0;
        //长连信令校验回包 
...
//    extern void SetShortlinkSvrAddr(const uint16_t port);
    ...
    extern bool (*ProxyIsAvailable)(const mars::comm::ProxyInfo& proxyInfo, const std::string& testHost, const std::vector<std::string>& hardCodeIps);
