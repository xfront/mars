...
void SdtCore::StartCheck(CheckIPPorts &_longlink_items, CheckIPPorts &_shortlink_items, int _mode, int _timeout) {
    ...
    if (thread_.isRunning() || thread_.start() != 0)
        return;
}
...
void SdtCore::__RunOn() {
    ...
    xinfo2(TSF"all checkers end! cancel_=%_, check_request_.check_status_=%_, check_list__size=%_", cancel_,
        ...
        switch (iter->netcheck_type) {
            case kTcpCheck:
                xinfo2(TSF"tcp check result, error_code:%_, ip:%_, port:%_, network_type:%_, rtt:%_",
                       iter->error_code, iter->ip, iter->port, iter->network_type, iter->rtt);
                break;
            case kHttpCheck:
            ...
            case kPingCheck:
                xinfo2(TSF"ping check result, error_code:%_, ip:%_, network_type:%_, loss_rate:%_, rtt:%_",
                       iter->error_code, iter->ip, iter->network_type, iter->loss_rate, iter->rtt_str);
                break;
            case kDnsCheck:
                xinfo2(TSF"dns check result, error_code:%_, domain_name:%_, network_type:%_, ip1:%_, rtt:%_",
                       iter->error_code, iter->domain_name, iter->network_type, iter->ip1, iter->rtt);
                break;
        }
    }
