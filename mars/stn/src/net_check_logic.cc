...
NetCheckLogic::NetCheckLogic()
        : mFrequencyLimit(new CommFrequencyLimit(kLimitCount, kLimitTimeSpan))
        , mLastNetcheckTime(0) {
    xinfo_function();
}
...
void NetCheckLogic::UpdateLongLinkInfo(unsigned int _continues_fail_count, bool _task_succ) {
    if (!_task_succ) mLongLinkTaskStatusItem.last_failedtime = ::gettickcount();
    SET_BIT(_task_succ, mLongLinkTaskStatusItem.records, kValidBitsFilter);
    xdebug2("shortlink:_continueFailCount=%d, _isTaskSucc=%d, records=0x%x", _continues_fail_count, _task_succ,
            mLongLinkTaskStatusItem.records);
...
void NetCheckLogic::UpdateShortLinkInfo(unsigned int _continues_fail_count, bool _task_succ) {
    ...
    if (!_task_succ) mShortLinkTaskStatusItem.last_failedtime = ::gettickcount();
    SET_BIT(_task_succ, mShortLinkTaskStatusItem.records, kValidBitsFilter);
    xdebug2("shortlink:_continues_fail_count: %d, _task_succ: %d, records=0x%x", _continues_fail_count, _task_succ,
            mShortLinkTaskStatusItem.records);
...
bool NetCheckLogic::__ShouldNetCheck() {
    ...
    EXTRACT_N_BITS(mShortLinkTaskStatusItem.records, kMostRecentTaskStartN[0], kMostRecentTaskStartN[1],
    ...
    if (is_shortlink_bad) { // 最近八次坏，前八次好，就进入netcheck
        ...
        EXTRACT_N_BITS(mShortLinkTaskStatusItem.records, kSecondRecentTaskStartN[0], kSecondRecentTaskStartN[1],
    ...
    EXTRACT_N_BITS(mLongLinkTaskStatusItem.records, kMostRecentTaskStartN[0], kMostRecentTaskStartN[1],
    ...
    if (is_longlink_bad) { // 最近八次坏，前八次好，就进入netcheck
        ...
        EXTRACT_N_BITS(mLongLinkTaskStatusItem.records, kSecondRecentTaskStartN[0], kSecondRecentTaskStartN[1],
    ...
    if (ret) {
        if (::gettickspan(mLastNetcheckTime) < (kMinCheckTimeSpan + increment_steps * kCheckTimeSpanIncrementStep)) {
            ret = false;
            xinfo2(TSF"continous hit netcheck strategy, skip this. mLastNetcheckTime=%_", mLastNetcheckTime);
        } else {
            increment_steps++;
        }
    }
...
void NetCheckLogic::__StartNetCheck() {
    ...
    for (std::vector<std::string>::iterator host_iter = longlink_hosts.begin();
        ...
        mDnsUtil.GetNewDNS().GetHostByName(*host_iter, longlink_iplist);
        if (longlink_iplist.empty()) mDnsUtil.GetDNS().GetHostByName(*host_iter, longlink_iplist);
    ...
    for (std::vector<std::string>::iterator iter = shortlink_hostlist.begin();
        ...
        mDnsUtil.GetNewDNS().GetHostByName(*iter, shortlink_iplist);
        if (shortlink_iplist.empty()) mDnsUtil.GetDNS().GetHostByName(*iter, shortlink_iplist);
