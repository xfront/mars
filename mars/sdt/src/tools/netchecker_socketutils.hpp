...
public:
        ...
        if (connectRet != 0) {
            ...
            while (true) {
                ...
                if (ret != 0) {
                    ...
                    } else xerror2(TSF"getsockopt error");
                    ::socket_close(fsocket);
                    return INVALID_SOCKET;
                }
     ...
     */
    static TcpErrCode
    writenWithNonBlock(int fdSocket, SocketSelect &sel, unsigned int timeoutMs, const unsigned char *buff,
                       unsigned int unSize, int &errcode) {
        xverbose_function();
     ...
     */
    static TcpErrCode
    readnWithNonBlock(int fdSocket, SocketSelect &sel, unsigned int timeoutMs, AutoBuffer &recvBuf, unsigned int unSize,
                      int &errcode) {
        xverbose_function();
        ...
        while (true) {
            ...
            if (sel.Read_FD_ISSET(fdSocket)) {
                ...
                if ((recvBuf.Length() - oldLength) >= unSize) {
                    xdebug2(TSF"recvBuf.Length()=%0, oldLength=%1, unSize=%2", recvBuf.Length(), oldLength,
                            unSize);
                    return kTcpNonErr;
                }
            }
