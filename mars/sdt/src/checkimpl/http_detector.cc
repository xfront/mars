...
#ifdef ANDROID
    snprintf(temp_time, sizeof(temp_time), "%d-%02d-%02d %+.1f %02d:%02d:%02d.%.3ld", 1900 + tm.tm_year, 1 + tm.tm_mon,
             tm.tm_mday,
...
void HTTPDetector::__Detect() {
            ...
            if (ret && !vec_ip.empty()) {
                ...
                if (result_.dns_cost_ >= remain_timeout) {
                    result_.dns_errmsg_.append(
                            std::string("GetHostByName timeout, set timeout:") + string_cast(remain_timeout).str() +
                            "ms");
                } else {
    ...
    if (remain_timeout <= 0) {
        xwarn2(TSF"@%_ HTTPDetect timeout, tcp_connect_cost_:%_, total_timeout:%_", this,
               result_.tcp_connect_cost_, req_.total_timeout_);
    ...
    if (breaker_.IsBreak()) {
        xwarn2(TSF"Send Request break, sent:%_ nread:%_, nwrite:%_", send_ret, socket_nread(sock),
               socket_nwrite(sock));
    ...
    if (remain_timeout <= 0) {
        xwarn2(TSF"@%_ HTTPDetect timeout, http_send_req_cost_:%_, total_timeout:%_", this,
               result_.http_send_req_cost_, req_.total_timeout_);
    ...
    while (true) {
        ...
        if (remain_timeout <= 0) {
            xwarn2(TSF"@%_ HTTPDetect timeout, span:%_, total_timeout:%_", this,
                   (int64_t) detect_tick.gettickspan(), req_.total_timeout_);
